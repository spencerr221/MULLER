...
# Source: https://github.com/activeloopai/deeplake/blob/94c5e100292c164b80132baf741ef233dd41f3d7/hub/core/compression.py
#
...
from muller.compression import (
    _compression_types,  # can be optimized.
    _JPEG_SOFS, _JPEG_SOFS_RE, _JPEG_SKIP_MARKERS, _STRUCT_HHB, _STRUCT_II
)
from muller.compression import (
...
# -------------------------- Obtain the compression information-----------------------
def get_compression_type(compression_name):
    """Returns the compression type for the given compression name."""
    if compression_name is None:
        return None
    ret = _compression_types.get(compression_name)
    if ret is None and compression_name.upper() in Image.OPEN:
        ret = "image"
    if ret is None:
        raise KeyError(compression_name)
    return ret


def get_compression(header=None, path=None):
    # Detect whether it is the desired file type as audio/video/dcm/nifti files.
    if path:
        path = str(path).lower()
        # Besides the image files recognized by Pillow, these are the formats that can be recognized by file extension:
        file_formats = [
            ".mp3", ".flac", ".wav",
            ".mp4", ".mkv", ".avi",
            ".dcm",
        ...
        for fmt in file_formats:
            if path.endswith(fmt):
                return fmt[1:]
    # Detect whether it is the desired file type as an image file. Note: The first 32 bytes will be enough.
    if header:
        if not Image.OPEN:
            Image.init()
...
# ------------------------- Read meta without fully decompression (called by sample) -------------------------
def read_meta_from_compressed_file(
    ...
    try:
        ...
        # Images: jpeg, png, dcm, nii/nii.gz
        ...
        # Audio: mp3, flac, wav
        elif get_compression_type(compression) == "audio":
            try:
...
def decompress_array(
    ...
    try:
        ...
        elif compr_type == "audio":
            return _decompress_audio(buffer)

        elif compr_type == "video":
        ...
        raise SampleDecompressionError(str(e), path) from e
    ...
    try:
        ...
        raise SampleDecompressionError(str(e), path)
...
def compress_array(array: np.ndarray, compression: Optional[str]) -> bytes:
    ...
    Raises:
        UnsupportedCompressionError: If `compression` is unsupported. See `muller.compressions`.
    ...
    if compression not in muller.compressions:
    ...
    if compr_type == "byte":
        ...
        raise NotImplementedError(
            "In order to store audio data, you should use `muller.read(path_to_file)` "
            "or specify sample_compression=None. "
            "Compressing raw data is not yet supported."
        )
    elif compr_type == "video":
        raise NotImplementedError(
            "In order to store video data, you should use `muller.read(path_to_file)` "
            "or specify sample_compression=None. "
            "Compressing raw data is not yet supported."
        )
    elif compr_type == "point_cloud":
        raise NotImplementedError(
            "In order to store point cloud data, you should use `muller.read(path_to_file)` "
            "or specify sample_compression=None. "
            "Compressing raw data is not yet supported."
        )
    elif compr_type == "mesh":
        raise NotImplementedError(
            "In order to store mesh data, you should use `muller.read(path_to_file)` "
            "or specify sample_compression=None. "
            "Compressing raw data is not yet supported."
        )
    elif compr_type == "nifti":
        raise NotImplementedError(
            "In order to store nifti data, you should use `muller.read(path_to_file)` "
            "or specify sample_compression=None. "
            "Compressing raw data is not yet supported."
        )
    if compression == "apng":
    ...
    try:
        if array.shape == (1, 1, 1):
            array = array[0]
        img = to_image(array)
        if compression.upper() == 'JPEG' and img.has_transparency_data:
            # JPEG not support transparency
            img = img.convert('RGB')
        out = BytesIO()
...
def _fast_decompress(buf):
    ...
    type_shape = Image._conv_type_shape(img)
    img.close()
    return type_shape
...
def decompress_bytes(
    ...
    if compression == "lz4":
        ...
        if buffer[:4] == b'\x04"M\x18':
            # python-lz4 magic number (backward compatibility)
...
def _open_audio(file: Union[str, bytes, memoryview]):
    if not _PYAV_INSTALLED:
        raise ModuleNotFoundError(
            "PyAV is not installed. Please run `pip install muller[audio]`"
        )
    if isinstance(file, str):
        container = av.open(
            file, metadata_errors='ignore', options={"protocol_whitelist": "file,http,https,tcp,tls,subfile"}
        )
    else:
        container = av.open(BytesIO(file), metadata_errors='ignore')
...
def _open_video(file: Union[str, bytes, memoryview]):
    if not _PYAV_INSTALLED:
        raise ModuleNotFoundError(
            "PyAV is not installed. Run `pip install muller[video]`."
        )
    if isinstance(file, str):
        container = av.open(
            file, metadata_errors='ignore', options={"protocol_whitelist": "file,http,https,tcp,tls,subfile"}
...
def _verify_jpeg_file(f):
    ...
    try:
        ...
        # Start of Image
        if soi != b"\xff\xd8":
            raise CorruptedSampleError("jpeg", f)
        ...
        if definition_start not in [b"\xff\xc4", b"\xff\xdb", b"\xff\xdd", b"\xff\xda"]:  # DHT,DQT,DRI,SOS
            raise CorruptedSampleError("jpeg", f)
        ...
        # TODO: (original) this check is too slow
        if mm.find(b"\xff\xd9") == -1:  # End of Image
            raise CorruptedSampleError("jpeg", f)
...
def _verify_jpeg(f):
    if hasattr(f, "read"):
        return _verify_jpeg_file(f), "|u1"
    return _verify_jpeg_buffer(f), "|u1"
...
def _verify_jpeg_buffer(buf: bytes):
    ...
    if mview[sof_idx + length + 2:sof_idx + length + 4] not in \
            [b"\xff\xc4", b"\xff\xdb", b"\xff\xdd", b"\xff\xda"]:  # DHT,DQT,DRI,SOS
        raise CorruptedSampleError("jpeg")
    ...
    if buf.find(b"\xff\xd9") == -1:  # EndofImage
        raise CorruptedSampleError("jpeg")
...
def _open_nifti(file: Union[bytes, memoryview, str], gz: bool = False):
    if not _NIBABEL_INSTALLED:
        raise ModuleNotFoundError(
            "nibabel is not installed. Please run `pip install muller[medical]`"
        )
