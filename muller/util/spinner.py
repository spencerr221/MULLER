...
def run_spinner(spinner):
    ...
    try:
        if not isinstance(sys.stderr, DummyFile) and muller.constants.SPINNER_ENABLED:
            spinner.start()
            ...
            logger = logging.getLogger("muller")
...
class Spinner(threading.Thread):
    ...
    def run(self):
        try:
            time.sleep(muller.constants.SPINNER_START_DELAY)
            frames = cycle("/-\\|")
